...
describe('Governance Integration Flow', () => {
  ...
  beforeEach(() => {
    ...
    // Save env
    originalEnv = {
      PI_GOV_USER: process.env['PI_GOV_USER'],
      PI_GOV_ROLE: process.env['PI_GOV_ROLE'],
      PI_GOV_ORG_UNIT: process.env['PI_GOV_ORG_UNIT'],
      PI_GOV_GOVERNANCE_CONFIG: process.env['PI_GOV_GOVERNANCE_CONFIG'],
    };
  });
  ...
  function setupEnv(role: string, user = 'test-user', orgUnit = 'default') {
    process.env['PI_GOV_USER'] = user;
    process.env['PI_GOV_ROLE'] = role;
    process.env['PI_GOV_ORG_UNIT'] = orgUnit;
    ...
    process.env['PI_GOV_GOVERNANCE_CONFIG'] = configPath;
  }
  ...
  it('config not found uses defaults — session starts successfully', async () => {
    // Don't set PI_GOV_GOVERNANCE_CONFIG — let it fall through
    delete process.env['PI_GOV_GOVERNANCE_CONFIG'];
    process.env['PI_GOV_USER'] = 'test-user';
    process.env['PI_GOV_ROLE'] = 'admin';
    process.env['PI_GOV_ORG_UNIT'] = 'default';
  ...
  it('DLP config hot-reloads', async () => {
    ...
    const configPath = process.env['PI_GOV_GOVERNANCE_CONFIG']!;
